...
$(document).ready(function () {
    ...
    /*$(".editEquipmentService").click(function () {
        ...
        if (TypeId == 1) {
            OpenRightToLeftLgModal("/Inventory/AddInventoryEquipment/?id=" + EditId);
        }
        else if (TypeId == 2) {
            OpenRightToLeftLgModal("/Inventory/AddNonInventory/?id=" + EditId);
        }
        else if (TypeId == 3) {
            OpenRightToLeftLgModal("/Inventory/AddService/?id=" + EditId);
        }
    });*/
    $(".inventory-edit-name").click(function (e) {
        e.preventDefault();
        if (cntrlIsPressed) {
            ...
            var TypeId = $(this).attr('data-id');
            if (TypeId == 2) {
                LoadServiceDetail(true,true, EditId);
            } else {
                LoadEquipmentDetail(true,true, EditId);
            }
    ...
    $(".equip-delete").click(function () {
        var selectedTotal = $(this).attr("data-total");
        if (selectedTotal > 0) {
            OpenCautionMessageNew("Warning!", "You can't delete this item.", function () {
                $(".close").trigger('click');
            });
        }
        else {
    ...
    $(".equip-assign").click(function () {
        var selectedTransferId = $(this).attr("data-id");
        OpenRightToLeftLgModal("Inventory/EquipmentAssign/?selectedTransferId=" + selectedTransferId);
    });
});
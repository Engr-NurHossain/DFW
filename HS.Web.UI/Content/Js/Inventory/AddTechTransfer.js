...
var PropertyUserSuggestiontemplate =
    '<div class="tt-suggestion tt-selectable" data-select="{1}" data-price="{2}" data-type="{5}" data-id="{0}" data-description="{6}" data-sku="{7}" data-onhand="{4}">'
    /*
    *For Equipment Image
...
//NewEquipmentRow += "@if (Model.QuantityOnHand.ToString().IndexOf(\" -\") > -1) {"
//    + "<td valign='top' class=''>"
...
//    + "</td>";
    ...
    //+ "</td> }"
    //+ "</td>";
NewEquipmentRow += "<td valign='top' class=''>"
...
var saveDataTextTemplate = '{1} | {2} | {3}  | {4}  | {5}  | {6}  | {7}  | {8}  | {9}  ';
...
var POEqSuggestionclickbind = function (item) {
    $('.CustomerInvoiceTab .tt-suggestion').click(function () {
        ...
        spnItemRate = $(item).parent().parent().find('.spnSKU');
        $(spnItemRate).text($(this).attr('data-sku'));
        txtItemRate = $(item).parent().parent().find('.txtSKU');
        $(txtItemRate).val($(this).attr('data-sku'));

        /*Item Quantity Set*/
        spnItemRate = $(item).parent().parent().find('.spnProductQuantity');
        $(spnItemRate).text(1);
        txtItemRate = $(item).parent().parent().find('.txtProductQuantity');
        $(txtItemRate).val(1);
        /*Item Amount Set*/
        spnItemRate = $(item).parent().parent().find('.spnProductAmount');
        $(spnItemRate).text($(this).attr('data-price'));
        txtItemRate = $(item).parent().parent().find('.txtProductAmount');
        $(txtItemRate).val($(this).attr('data-price'));
        /*Item QuantityOnHand Set*/
        spnItemRate = $(item).parent().parent().find('.spnProductQuantityOnHand');
        $(spnItemRate).text($(this).attr('data-onhand'));
        if (parseInt($(this).attr('data-onhand')) < 0) {
            $(spnItemRate).text(0);
        }
        else {
            $(spnItemRate).text($(this).attr('data-onhand'));
        ...
        if (parseInt($(this).attr('data-onhand')) == 0) {
            ...
            $(spnItemRate).parent().removeClass("qtyhandwarning");
        //console.log($(this).attr('data-onhand'));
        /*Item ProductQuantity Set*/
        spnItemRate = $(item).parent().parent().find('.spnProductQuantity');
        $(spnItemRate).text(1);
        txtItemRate = $(item).parent().parent().find('.txtProductQuantity');
        $(txtItemRate).val(1);
        ...
        txtItemRate = $(item).parent().parent().find('.txtProductQuantityReceived');
        $(txtItemRate).val(0);
        /*Item QuantityReceiving Set*/
        spnItemRate = $(item).parent().parent().find('.spnProductQuantityReceiving');
        $(spnItemRate).text(0);
        txtItemRate = $(item).parent().parent().find('.txtProductQuantityReceiving');
        $(txtItemRate).val(0);
...
var SearchKeyUp_old = function (item, event) {
            ...
            if (resultparse.length > 0) {
                ...
                for (var i = 0; i < resultparse.length; i++) {
                    searchresultstring = searchresultstring + String.format(PropertyUserSuggestiontemplate,
                        ...
                        /*2*/ resultparse[i].SupplierCost,
                        /*3*/resultparse[i].Reorderpoint,
                        /*4*/ resultparse[i].QuantityOnHand,
                        /*5*/ resultparse[i].EquipmentType,
                        /*6*/resultparse[i].EquipmentDescription.replaceAll('"', '\'\''),
                        /*7*/resultparse[i].SKU,
                        /*8*/resultparse[i].ManufacturerName.replaceAll('"', '\'\'')/*ImageSource*/);
...
var SaveTechTransferRequest = function (OpenEmail) {
    //console.log("SaveTechTransferRequest");
    var Proceed = true;
    if ($(".HasItem").length == 0) {
    ...
    if ($('#TechFromList').val() == $('#TechToList').val()) {
        OpenErrorMessageNew("Error!", "You're transferring items to yourself, select another technician.");
        Proceed = false;
        return 0;
    }


    var url = domainurl + "/Inventory/TechTransferRequest";
    var DetailList = [];
    var Items = [];
    var ReceivingQuantity = 0;
    var i = 0;
    $(".HasItem").each(function (e, item) {
        var TotalPrice = $(item).find('.txtProductQuantity').val();
        DetailList.push({
            EquipmentId: $(item).attr('data-id'),
            EquipName: $(item).find('.ProductName').val(),
            SKU: $(item).find('.txtSKU').val(),
            OnHand: $(item).find('.spnProductQuantityOnHand').text(),
        ...
        Items.push({
            ...
            Quantity: $(item).find('.txtProductQuantity').val(),
            IsReceived: false,
            ReceivedDate: new Date($('#TransferDate').val()),
            ReceivedBy:  $('#TechToList').val(),
            CreatedDate:  new Date(),
            IsApprove: false,
            IsDecline: false
        });

        //console.log(String.format(saveDataTextTemplate,
        //                /*0*/DetailList[i].EquipmentId,
        //                /*1*/ DetailList[i].EquipName,
        //                /*2*/ DetailList[i].SKU,
        //                /*3*/DetailList[i].Quantity,
        //                /*4*/ DetailList[i].OnHand,
        //                /*5*/ DetailList[i].TotalPrice,
        //                /*6*/DetailList[i].RecieveQty,
        //                /*7*/DetailList[i].TransferDate,
        //                /*8*/DetailList[i].CurrentQty));

        //console.log(DetailList[i].OnHand);
        //console.log(DetailList[i].Quantity);
        //if (DetailList[i].OnHand < DetailList[i].Quantity) console.log("True"); else console.log("False");
        //if (parseInt(DetailList[i].OnHand) < parseInt(DetailList[i].Quantity)) console.log("True"); else console.log("False");
        if (parseInt(DetailList[i].OnHand) < parseInt(DetailList[i].Quantity)) {
            OpenErrorMessageNew("Error!", "Transfer quantity can't be more than OnHand quantity.");
            Proceed = false;
            return 0;
        }

        if (parseInt(DetailList[i].Quantity) <= 0) {
...
$(document).ready(function () {
    ...
    // from tech dropdown changes existing equipment row clear and adding new empty row : mayur

    $("#TechFromList").change(function () {

        $(".CustomerInvoiceTab tbody tr").remove();

        // add new row

        $("#CustomerInvoiceTab tbody").append(NewEquipmentRow);
    ...
    $(".CustomerInvoiceTab tbody").on('change', "tr td .txtProductQuantity", function () {
        ...
        if (!Receiving) {
            ...
            console.log("ProductRateDom val",ProductRateDom);
    ...
    $(".CustomerInvoiceTab tbody").on('change', "tr td .txtProductQuantityReceived", function () {
        var ProductQuantityDom = $(this).parent().find('span.spnProductQuantityOnHand');
        $(ProductQuantityDom).text($(this).val());
        var ProductRateDom = $(this).parent().parent().find('input.txtProductRate');
        if ($(ProductRateDom).val() != "" && parseFloat($(ProductRateDom).val().trim().replaceAll(',', '')) > 0) {
            var NewProductAmount = $(this).val() * parseFloat($(ProductRateDom).val().trim().replaceAll(',', ''));

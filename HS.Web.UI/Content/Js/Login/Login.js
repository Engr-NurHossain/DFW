    ...
    var url = domainurl + "/Login/LoginAjax/";
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.msg == "reset") {
                    $("#updatepassworddays").text(data.updatedays);
                    $("#btnResetPasswor").click(); 
...
$(document).ready(function () {
    setTimeout(function () {
        if (typeof $.cookie('_UserInfo_') != 'undefined' && $.cookie('_UserInfo_') != null) {
            var decryptedBytes = CryptoJS.AES.decrypt($.cookie('_UserInfo_'), "My Secret Passphrase");
            var plaintext = decryptedBytes.toString(CryptoJS.enc.Utf8);
            if (plaintext != "" && plaintext.indexOf("||") > -1) {
                var Vals = plaintext.split("||");
                $("#UserName").val(Vals[0]);
                $("#Password").val(Vals[1]);
            }
        }

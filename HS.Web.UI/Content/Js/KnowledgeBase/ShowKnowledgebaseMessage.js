var CheckFlag = function (Id, flg) {
    ...
    $.ajax({
        type: "POST",
        url: domainurl + "/Sales/FlagedArtical",
        data: modelval,
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        success: function (data) {
            console.log("Reload");
            parent.$(".KnowledgeBaseMessage").attr("href", "/Sales/ShowKnowledgebaseMessage?Id=" + data.Id + "&SearchText=" + SearchText);
            parent.$(".KnowledgeBaseMessage").click();
        },
...
var KnowledgebaseFileList = function (order, Id) {
    console.log("HHHHHHH");
    var url = "/Sales/SortKnowledgebaseFileList";
...
function GetRecentComment() {
    ...
    $.ajax({
        url: '/Sales/GetCommentForKnowledge',
        type: 'post',
        data: { id: $("#Id").val() },
    ...
    var x = $.ajax({
        ...
        success: function (data) {
            if (flg) {
                ...
                $('#comment_section_save').removeAttr('disabled');
            } else {
...
var AssignKnowledgebase = function (Id) {
    console.log("TTT");
    $(".AssignKnowledge").attr("href", "/Sales/AssignKnowledgebaseToUser?Id=" + Id);
    $(".AssignKnowledge").click();
};
var ClosePopup = function () {
    $.magnificPopup.close();
}
var AddKnowledgebase = function (id) {
    window.location.href = domainurl + "/knowledgebase?isdocumentlibrary=false" + "&knowledgebaeid=" + id;
}

var IsReadAritcle = function (Id, isread) {
    var modelval = JSON.stringify({
            Id: Id,
            IsRead: isread,
        });
        var x = $.ajax({
            type: "POST",
            url: domainurl + "/Sales/ReadAssignKnowledgebaseByUser",
            data: modelval,
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            success: function (data) {
                window.location.href = domainurl + "/knowledgebase/Id=" + data.Id;
            },
            error: function (jqXHR, textStatus, errorThrown) {
                console.log(errorThrown);
...
$(document).ready(function () {
    ...
    $("#favorite").change(function () {
        ...
        var Id = $("#knowledgebaseId").val();
        var favorite = $("#favorite").is(":checked");
        var url = domainurl + "/Leads/SetAsFavorite?Id=" + Id + "&IsFavourite=" + favorite;
        $.ajax({
            type: "POST",
            url: url,
            dataType: "json",
    ...
    var idlist = [{ id: ".DocPreview", type: 'iframe', width: Popupwidth, height: ConditionHeight },
        { id: ".AssignKnowledge", type: 'iframe', width: Popupwidth -520, height: ConditionHeight -100 }
    ];
    ...
    $(".Flag2").change(function () {
        ...
        if (flg) {
            ...
            console.log("obayed",RemovedFlag.toLowerCase());
            if (RemovedFlag.toLowerCase() == "true") {
                CheckFlag2(Id, flg);
            }
            else {
    ...
    $("#IsRead").change(function () {
        ...
        var Id = $("#Id").val();
        IsReadAritcle(Id, isread);
    });
    $("#comment_section_save").click(function () {
        $('#comment_section_save').attr("disabled", "disabled");
    ...
    $(".icon_sort_timeclock").click(function () {
        ...
        KnowledgebaseFileList(orderval, Id);
    });
});

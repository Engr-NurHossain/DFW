...
var NewAccountInformationRow = "<tr>"
                        + "<td valign='top' class='rowindex'></td>"
                        ...
                        + "</td>"
                        + "<td valign='top'>"
                            + "<input type='text' class='txtAccountDesc' />"
                            + "<span class='spnAccountDesc'></span>"
                        + "</td>"
                        + "<td valign='top'>"
                            + "<input type='text' class='txtAccountAmount' />"
...
var InvoiceEqSuggestionclickbind = function (item) {
    $('.CustomerInvoiceTab .tt-suggestion').click(function () {
        ...
        $(txtItemRate).val(1);
        var spnItemRate = $(item).parent().parent().find('.spnAccountAmount');
        $(spnItemRate).text($(this).attr('data-price'));
...
var SupplierAccountTypeSuggestionClickBind = function (item) {
    $('.AccountDescriptionTab .tt-suggestion').click(function () {
        var clickitem = this;
        $('.AccountDescriptionTab .tt-menu').hide();
        $(item).val($(clickitem).attr('data-select'));
        $(item).attr('data-id', $(clickitem).attr('data-id'));
        ...
        //var spnItemRate = $(item).parent().parent().find('.spnAccountDesc');
        //$(spnItemRate).text($(this).attr('data-description'));
        //var txtItemRate = $(item).parent().parent().find('.txtAccountDesc');
        ...
        //$(spnItemRate).text($(this).attr('data-price'));
        //var txtItemRate = $(item).parent().parent().find('.txtAccountAmount');
        //$(txtItemRate).val($(this).attr('data-price'));

        CalculateNewAmount();

    });
    $('.AccountDescriptionTab .tt-suggestion').hover(function () {
        var clickitem = this;
        $('.tt-suggestion').removeClass("active");
        $(clickitem).addClass('active');
    });
...
var SearchAccountKeyUp = function (item, event) {
    if (event.keyCode == 40 || event.keyCode == 38 || event.keyCode == 13)
        return false;
    $.ajax({
        url: domainurl + "/Expense/GetAccountTypeListByKey",
        data: {
            key: $(item).val()
        },
            ...
            if (resultparse.length > 0) {
                ...
                for (var i = 0; i < resultparse.length; i++) {
                    searchresultstring = searchresultstring + String.format(AccountTypeSuggestiontemplate,
                        /*0*/resultparse[i].Id,
                        /*1*/resultparse[i].Name,/*2*/ resultparse[i].Type);
                }
                ...
                SupplierAccountTypeSuggestionClickBind(item);
...
var SearchAccountKeyDown = function (item, event) {
...
var CalculateNewAmount = function () {
    ...
    $(".amount").text(TransCurrency + amount);
    TotalAmount = amount;
}
...
$(document).ready(function () {
    ...
    $(".AccountDescriptionTab tbody").sortable({
        update: function () {
            var i = 1;
            $(".AccountDescriptionTab tbody tr td:first-child").each(function () {
                $(this).text(i);
                i += 1;
            });
        }
    ...
    $("#AccountDescriptionTab tbody").on('click', 'tr', function (e) {
        if ($(e.target).hasClass('fa')) {
            return;
        }
        $("#AccountDescriptionTab tr").removeClass("focusedItem");
        $(this).addClass("focusedItem");
    ...
    $("#AccountDescriptionTab tbody").on('click', 'tr:last', function (e) {
        if ($(e.target).hasClass('fa')) {
            return;
        }
        $("#AccountDescriptionTab tbody tr:last").after(NewAccountInformationRow);
        var i = 1;
        $(".AccountDescriptionTab tbody tr td:first-child").each(function () {
            $(this).text(i);
            i += 1;
    ...
    $(".AccountDescriptionTab tbody").on('click', 'tr td i.fa', function (e) {
        $(this).parent().parent().remove();
        if ($(".AccountDescriptionTab tbody tr").length < 2) {
            $("#AccountDescriptionTab tbody tr:last").after(NewAccountInformationRow);
        }
        var i = 1;
        $(".AccountDescriptionTab tbody tr td:first-child").each(function () {

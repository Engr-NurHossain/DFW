...
var CityStateSearchKeyUp = function (item, event) {
    ...
    if ($(item).val().toString().length >= 3) {
        $.ajax({
            url: "/App/GetCityStateZipListByKey",
            data: {
                key: $(item).val()
            },
            ...
            success: function (data) {
                ...
                if (resultparse.length > 0) {
                    ...
                    if (resultparse.length > 5) {
                        $(".CityState_insert_div .CityState_insert_div_previous .CityState_insert_div_additional .CityStateZipSuggestion").height(200);
...
var CityStateclickbind = function (item) {
    ...
    $('.CityState_insert_div_additional .tt-suggestion').click(function () {
        var clickitem = this;
        $('.CityState_insert_div_additional .tt-menu').hide();
        var selectedZip = $(clickitem).attr("data-zip").trim();
        $("#ZipCodeAdditional").val(selectedZip);
        var selectedCity = $(clickitem).attr("data-city").trim();
        $("#CityAdditional").val(selectedCity);
        var selectedState = $(clickitem).attr("data-state").trim();
        $("#StateAdditional").val(selectedState);
    });
    $('.CityState_insert_div .CityState_insert_div_previous .CityState_insert_div_additional .tt-suggestion').hover(function () {
        var clickitem = this;
...
$(document).ready(function () {
    ...
    $("#CorpZipCode").focusout(function () {
        setTimeout(function () {
            $(".CityState_insert_div .CityState_insert_div_previous .CityState_insert_div_additional .tt-menu").hide();
        }, 200);
    });
    ...
    $("#ZipCodeAdditional").focusout(function () {
        setTimeout(function () {
            $(".CityState_insert_div .CityState_insert_div_previous .CityState_insert_div_additional .tt-menu").hide();
        }, 200);
    });

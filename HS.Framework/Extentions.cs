...
namespace HS.Framework
{
    public static class Extentions
    {
        ...
        public static string PhoneNumberFormatNew(this string Num)
        {
        ...
        public static string TrimPhoneNum(this string PhoneNo)
        {
            string FormatedPhoneNo = "";
            FormatedPhoneNo = PhoneNo.Replace("-", "").Replace("+", "").Replace("(", "").Replace(")", "").Replace(" ", "");
            return FormatedPhoneNo;
        }
        public static string FormatedPhoneNum(this string PhoneNo)
        {
            double Num = 0;
            string FormatedPhoneNo = "";
            FormatedPhoneNo = PhoneNo.Replace("-", "").Replace("+", "").Replace("(", "").Replace(")", "").Replace(" ", "");
            bool isNum = double.TryParse(FormatedPhoneNo, out Num);
            if (FormatedPhoneNo.Length == 10 && isNum == true)
            {
                FormatedPhoneNo = string.Format("({0}) {1}-{2}", FormatedPhoneNo.Substring(0, 3), FormatedPhoneNo.Substring(3, 3), FormatedPhoneNo.Substring(6));
            }
            return FormatedPhoneNo;
        }
        ...
        public static float DoubleRound(this float value, int declength)
        {
            float result = value;
            if (value.ToString().Contains("."))
            {
                string full = value.ToString().Split(".")[0];
                string dec = value.ToString().Split(".")[1];
                if (!string.IsNullOrWhiteSpace(dec) && dec.Length > declength)
                {
                    int th = int.Parse(dec.Substring(declength, 1));
                    dec = dec.Substring(0, declength);
                    float.TryParse(string.Format("{0}.{1}", full.Trim(), dec.Trim()), out result);
                    if (th > 4)
                    {
                        if (declength == 2)
                        {
        ...
        public static decimal DoubleRound(this decimal value, int declength)
        {
            ...
            if (value.ToString().Contains("."))
            {
                ...
                if (!string.IsNullOrWhiteSpace(dec) && dec.Length > declength)
                {
                    int th = int.Parse(dec.Substring(declength, 1));
                    dec = dec.Substring(0, declength);
                    decimal.TryParse(string.Format("{0}.{1}", full.Trim(), dec.Trim()), out result);
                    if (th > 4)
                    {
                        if (declength == 2)
                        {
        ...
        public static double DoubleRound(this double value, int declength)
        {
            double result = value;
            if (value.ToString().Contains("."))
            {
                ...
                if (!string.IsNullOrWhiteSpace(dec) && dec.Length > declength)
                {
                    int th = int.Parse(dec.Substring(declength, 1));
                    dec = dec.Substring(0, declength);
                    double.TryParse(string.Format("{0}.{1}", full.Trim(), dec.Trim()), out result);
                    if (th > 4)
                    {
                        if (declength == 2)
                        {

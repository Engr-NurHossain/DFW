...
namespace HS.DataAccess
{
	public partial class GlobalSettingDataAccess : BaseDataAccess
	{
		...
		{	
			...
			AddParameter(cmd, pNVarChar(GlobalSettingBase.Property_Tag, 50, globalSettingObject.Tag));
			AddParameter(cmd, pNVarChar(GlobalSettingBase.Property_InputType, 50, globalSettingObject.InputType));
			AddParameter(cmd, pNVarChar(GlobalSettingBase.Property_Description, globalSettingObject.Description));
			AddParameter(cmd, pInt32(GlobalSettingBase.Property_OrderBy, globalSettingObject.OrderBy));
			AddParameter(cmd, pNVarChar(GlobalSettingBase.Property_OptionalValue, globalSettingObject.OptionalValue));
		}
		#endregion
        ...
        //public DataTable GetAlarmSearchkeyandValue()
        //{
        //    string sqlQuery = @"select Id,SearchKey,Value from GlobalSetting where SearchKey like'%Alarm%'";
        //    try
        //    {
		...
		protected void FillObject(GlobalSettingBase globalSettingObject, SqlDataReader reader, int start)
		{
				...
				if(!reader.IsDBNull(5)) globalSettingObject.Tag = reader.GetString( start + 5 );			
				if(!reader.IsDBNull(6)) globalSettingObject.InputType = reader.GetString( start + 6 );			
				if(!reader.IsDBNull(7)) globalSettingObject.Description = reader.GetString( start + 7 );			
				if(!reader.IsDBNull(8)) globalSettingObject.OrderBy = reader.GetInt32( start + 8 );			
				if(!reader.IsDBNull(9)) globalSettingObject.OptionalValue = reader.GetString( start + 9 );			
			FillBaseObject(globalSettingObject, reader, (start + 10));

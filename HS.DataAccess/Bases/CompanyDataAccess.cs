...
namespace HS.DataAccess
{
	public partial class CompanyDataAccess : BaseDataAccess
	{
		...
		{	
			...
			AddParameter(cmd, pDateTime(CompanyBase.Property_CreatedDate, companyObject.CreatedDate));
			AddParameter(cmd, pGuid(CompanyBase.Property_CreatedBy, companyObject.CreatedBy));
		}
		#endregion
		...
		protected void FillObject(CompanyBase companyObject, SqlDataReader reader, int start)
		{
				...
				companyObject.CreatedDate = reader.GetDateTime( start + 18 );			
				companyObject.CreatedBy = reader.GetGuid( start + 19 );			
			FillBaseObject(companyObject, reader, (start + 20));

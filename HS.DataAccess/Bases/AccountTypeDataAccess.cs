...
namespace HS.DataAccess
{
	public partial class AccountTypeDataAccess : BaseDataAccess
	{
		...
		{	
			AddParameter(cmd, pNVarChar(AccountTypeBase.Property_Name, 250, accountTypeObject.Name));
			AddParameter(cmd, pNVarChar(AccountTypeBase.Property_Type, 250, accountTypeObject.Type));
		}
		#endregion
		...
		protected void FillObject(AccountTypeBase accountTypeObject, SqlDataReader reader, int start)
		{
				...
				if(!reader.IsDBNull(1)) accountTypeObject.Name = reader.GetString( start + 1 );			
				if(!reader.IsDBNull(2)) accountTypeObject.Type = reader.GetString( start + 2 );			
			FillBaseObject(accountTypeObject, reader, (start + 3));
